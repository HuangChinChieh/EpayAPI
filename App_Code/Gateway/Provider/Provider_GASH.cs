...
public class Provider_GASH : GatewayCommon.ProviderGateway
{
    ...
    Dictionary<string, string> GatewayCommon.ProviderGateway.GetSubmitData(GatewayCommon.Payment payment)
    {
        ...
        PayDB.InsertPaymentTransferLog("交易開始 request:" + GetSendData(xmlDoc), 1, payment.PaymentSerial, payment.ProviderCode);
	...
	<soap:Envelope xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"" xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"">
	  <soap:Body>
        ...
        PayDB.InsertPaymentTransferLog("查單開始 request:" + GetSendData(xmlDoc), 1, payment.PaymentSerial, payment.ProviderCode);
        ...
        PayDB.InsertPaymentTransferLog("查單結束 response:" + InnerText, 1, payment.PaymentSerial, payment.ProviderCode);
        ...
        try
        {
            if (!string.IsNullOrEmpty(InnerText))
            {
                ...
                {
                    PayDB.InsertPaymentTransferLog("反查订单回传有误:訂單類型有誤:" + xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                    Ret.IsPaymentSuccess = false;
                    return Ret;
                }
                else
                {
                    ...
                    {
                        PayDB.InsertPaymentTransferLog("反查订单回传有误:簽名錯誤" + xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                        Ret.IsPaymentSuccess = false;
                        return Ret;
                    }
                    else
                    {
                        ...
                        {
                            PayDB.InsertPaymentTransferLog("反查订单回传有误:訂單編號不符:" + xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                            Ret.IsPaymentSuccess = false;
                            return Ret;
                        }
                        else
                        {
                            ...
                            {
                                PayDB.InsertPaymentTransferLog("反查订单成功", 1, payment.PaymentSerial, payment.ProviderCode);
                            ...
                            {
                                PayDB.InsertPaymentTransferLog("反查订单回传有误:訂單為失敗單:" + orderPaymentStatus, 1, payment.PaymentSerial, payment.ProviderCode);
                                Ret.IsPaymentSuccess = false;
                                return Ret;
                            }
                            //Order in Process
                            ...
                            {
                                PayDB.InsertPaymentTransferLog("反查订单回传有误:訂單狀態有誤:" + orderPaymentStatus, 1, payment.PaymentSerial, payment.ProviderCode);
                                Ret.IsPaymentSuccess = false;
                                return Ret;
                            }
                            else {
                                PayDB.InsertPaymentTransferLog("反查订单回传有误:訂單處理中:" + orderPaymentStatus, 1, payment.PaymentSerial, payment.ProviderCode);
                                Ret.IsPaymentSuccess = false;
                                return Ret;
                            }
                            #endregion
	...
	<soap:Envelope xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"" xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"">
	  <soap:Body>
        ...
        PayDB.InsertPaymentTransferLog("請款開始 request:" + GetSendData(xmlDoc), 1, payment.PaymentSerial, payment.ProviderCode);
        ...
        PayDB.InsertPaymentTransferLog("請款結束 response:" + InnerText, 1, payment.PaymentSerial, payment.ProviderCode);
        ...
        try
        {
            if (!string.IsNullOrEmpty(InnerText))
            {
                ...
                {
                    PayDB.InsertPaymentTransferLog("請款失敗:訂單類型有誤:" + xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                    Ret.IsPaymentSuccess = false;
                    return Ret;
                }
                else
                {
                    ...
                    if (!CodingControl.VerifyERPC(ref xmlDoc, xmlDoc.DocumentElement["ERPC"].InnerText, _TransactionKey))
                    {
                        PayDB.InsertPaymentTransferLog("請款失敗:簽名有誤:" + xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                        Ret.IsPaymentSuccess = false;
                        return Ret;
                    }
                    else
                    {
                        ...
                        if (payment.PaymentSerial != xmlDoc.DocumentElement["COID"].InnerText)
                        {
                            PayDB.InsertPaymentTransferLog("請款失敗:訂單編號有誤:" + xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                            Ret.IsPaymentSuccess = false;
                            return Ret;
                        }
                        else
                        {
                            ...
                            if (settleStatus == "s")
                            {
                                PayDB.InsertPaymentTransferLog("请款成功:"+ xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                                Ret.IsPaymentSuccess = true;
                                ...
                                PayDB.InsertPaymentTransferLog("请款失敗:" + xmlDoc.OuterXml, 1, payment.PaymentSerial, payment.ProviderCode);
                                Ret.IsPaymentSuccess = false;
                                return Ret;
                            }
                        }
            ...
            {
                PayDB.InsertPaymentTransferLog("请款回传有误:回传为空值", 1, payment.PaymentSerial, payment.ProviderCode);
                Ret.IsPaymentSuccess = false;
                return Ret;
            }
        }
