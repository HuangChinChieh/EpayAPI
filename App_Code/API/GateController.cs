...
public class GateController : ApiController
{
    ...
    public HttpResponseMessage RequirePaying([FromBody] FromBodyRequirePaying frombody)
    {
        ...
        #region 渠道总开关检查
        DT = RedisCache.WebSetting.GetWebSetting("WebsiteOption");
        if (!(DT != null && DT.Rows.Count > 0))
        {
            PayDB.InsertDownOrderTransferLog("站台維護中", 0, "", body.OrderID, body.CompanyCode, true);
            response = Request.CreateResponse(HttpStatusCode.Moved);
        ...
        if (PaymentModel.ProviderCode == "TestProvider")
        {
        ...
        else if (PaymentModel.ServiceType == "JOOB08") {
            var bindingBankCard = PayDB.spPaymentBindingBankCard(PaymentModel.PaymentID, 0, PaymentModel.OrderAmount, PaymentModel.OrderAmount, PaymentModel.UserName);
            if (bindingBankCard == 0)
            {
                response.Headers.Location = new Uri("/RedirectView/TransferInformation.cshtml?PaymentSerial=" + PaymentModel.PaymentSerial, UriKind.Relative);
            }
            else {
                response.Headers.Location = new Uri("/RedirectView/TransferInformation.cshtml?PaymentSerial=" + bindingBankCard, UriKind.Relative);
            }
            ...
            if (bindingBankCard == 0) {
                ...
                response.Headers.Location = new Uri("/RedirectView/TransferInformation_Alipay.cshtml?PaymentSerial=" + PaymentModel.PaymentSerial, UriKind.Relative);
            } else {
                response.Headers.Location = new Uri("/RedirectView/TransferInformation_Alipay.cshtml?PaymentSerial=" + bindingBankCard, UriKind.Relative);
            }
    ...
    public HttpResponseMessage RequirePayingUrl([FromBody] FromBodyRequirePaying frombody)
    {
        ...
        if (!(DT != null && DT.Rows.Count > 0))
        {
            PayDB.InsertDownOrderTransferLog("站台維護中", 0, body.OrderID, body.OrderID, body.CompanyCode, true);
            paymentResult.Status = GatewayCommon.ResultStatus.ERR;
            paymentResult.Message = "站台維護中";
            response.Content = new StringContent(Newtonsoft.Json.JsonConvert.SerializeObject(paymentResult));
            return response;
        }
        ...
        if (DT.Rows[0]["SettingValue"].ToString() == "1")
        {
            PayDB.InsertDownOrderTransferLog("站台維護中!", 0, body.OrderID, body.OrderID, body.CompanyCode, true);
            paymentResult.Status = GatewayCommon.ResultStatus.ERR;
            paymentResult.Message = "站台維護中!" + CodingControl.GetUserIP();
            response.Content = new StringContent(Newtonsoft.Json.JsonConvert.SerializeObject(paymentResult));
            return response;
        }
        #endregion
        ...
        if (PaymentModel.ServiceType== "JOOB08")
        {
            var bindingBankCard = PayDB.spPaymentBindingBankCard(PaymentModel.PaymentID, 0, PaymentModel.OrderAmount, PaymentModel.OrderAmount, PaymentModel.UserName);
            if (bindingBankCard == 0)
            {
                ...
                paymentResult.Url = baseUrl + "/RedirectView/TransferInformation.cshtml?PaymentSerial=" + PaymentModel.PaymentSerial;
            }
            else {
                PayDB.InsertDownOrderTransferLog("取得充值網址失敗"+ bindingBankCard, 0, PaymentSerial, body.OrderID, body.CompanyCode, true);
                paymentResult.Status = GatewayCommon.ResultStatus.ERR;
            ...
            if (bindingBankCard == 0) {
                ...
                paymentResult.Url = baseUrl + "/RedirectView/TransferInformation_Alipay.cshtml?PaymentSerial=" + PaymentModel.PaymentSerial;
            } else {
                PayDB.InsertDownOrderTransferLog("取得充值網址失敗" + bindingBankCard, 0, PaymentSerial, body.OrderID, body.CompanyCode, true);
                paymentResult.Status = GatewayCommon.ResultStatus.ERR;
                paymentResult.Message = "取得充值網址失敗";
            ...
            paymentResult.Url = baseUrl + "/RedirectView/GwtRedirect.cshtml?PaymentSerial=" + PaymentModel.PaymentSerial;
        }
    ...
    public HttpResponseMessage RequireWithdraw([FromBody] FromBodyWithdrawRequire frombody)
    {
        ...
        try
        {
            ...
            if (!(DT != null && DT.Rows.Count > 0))
            {
                PayDB.InsertDownOrderTransferLog("站台維護中", 2, "", body.OrderID, body.CompanyCode, true);
                withdrawResult.Status = GatewayCommon.ResultStatus.ERR;
                withdrawResult.Message = "站台維護中";
                response.Content = new StringContent(Newtonsoft.Json.JsonConvert.SerializeObject(withdrawResult));
            ...
            if (!Pay.IsTestSite)
            {
                var UserIP = CodingControl.GetUserIP();
                if (PayDB.GetWithdrawalIP(UserIP, body.CompanyCode) <= 0)
                {
                    PayDB.InsertDownOrderTransferLog("该IP未在白名单内。 IP：" + UserIP + ",公司代码：" + body.CompanyCode, 2, "", body.OrderID, body.CompanyCode, true);
                    withdrawResult.Status = GatewayCommon.ResultStatus.ERR;
                    withdrawResult.Message = "该IP未在白名单内。 IP：" + UserIP;
                    response.Content = new StringContent(Newtonsoft.Json.JsonConvert.SerializeObject(withdrawResult));
            ...
            else
            {
                ...
                if ((body.OrderAmount + CompanyWithdrawLimitModel.Charge+ (body.OrderAmount *0.01M* CompanyWithdrawLimitModel.Rate)) > CompanyPointModel.CanUsePoint - CompanyPointModel.FrozenPoint)
                {
                    PayDB.InsertDownOrderTransferLog("商户可用余额不足", 2, "", body.OrderID, body.CompanyCode, true);
                    withdrawResult.Status = GatewayCommon.ResultStatus.ERR;
                    withdrawResult.Message = "商户可用余额不足";
                ...
                if (CompanyModel.WithdrawType == 1)
                {
                    ...
                    //檢查公司可用餘額，專用渠道才有用，正常公司下發值必定大於供應商
                    if ((body.OrderAmount + CompanyWithdrawLimitModel.Charge+ (body.OrderAmount * 0.01M * CompanyWithdrawLimitModel.Rate)) > CompanyServicePointModel.CanUsePoint - CompanyServicePointModel.FrozenPoint)
                    {
                        PayDB.InsertDownOrderTransferLog("商户支付通道余额不足", 2, "", body.OrderID, body.CompanyCode, true);
                        withdrawResult.Status = GatewayCommon.ResultStatus.ERR;
                        withdrawResult.Message = "商户支付通道余额不足";
                ...
                if (CompanyModel.WithdrawType == 1)//自动代付
                {
                    ...
                    else
                    {
                        ...
                        WithdrawModel = new GatewayCommon.Withdrawal()
                        {
                            ...
                            Amount = body.OrderAmount,
                            CollectCharge = CompanyWithdrawLimitModel.Charge,
                            DownRate = CompanyWithdrawLimitModel.Rate,
    ...
    public class QueryWithdrawResult : APIResult
    {
        ...
        public void SetByWithdraw(GatewayCommon.Withdrawal withdrawal, GatewayCommon.WithdrawResultStatus withdrawalStatus, string CompanyCode, string CompanyKey)
        {
            ...
            this.WithdrawCharge = withdrawal.CollectCharge + (withdrawal.DownRate * withdrawal.Amount * 0.01M);
