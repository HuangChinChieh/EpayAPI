...
public class CallBackController : ApiController
{
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("JiuYuanPayNotify")]
    public HttpResponseMessage JiuYuanPayNotify([FromBody] _JiuYuanPayAsyncNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("JiuYuanPay");
        ...
        if (NotifyBody != null)
        {
            ...
            paymentSerial = NotifyBody.fxddh;
            #endregion
        ...
        try
        {
            ...
            #region 簽名檢查
            string secret = providerSetting.MerchantKey;

            string signStr = "fxstatus=" + NotifyBody.fxstatus +
                 "&fxid=" + providerSetting.MerchantCode +
                 "&fxddh=" + NotifyBody.fxddh +
                 "&fxfee=" + NotifyBody.fxfee +
                 "&" + secret;


            sign = CodingControl.GetMD5(signStr, false);

            if (sign != NotifyBody.fxsign)
            {
            ...
            #endregion
            if (paymentModel.OrderAmount != decimal.Parse(NotifyBody.fxfee))
            {
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.fxfee), NotifyBody.fxorder))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("JiuYuanPayWithdrawNotify")]
    public HttpResponseMessage JiuYuanPayWithdrawNotify([FromBody] _JiuYuanPayWAsyncNotifyBody NotifyBody)
    {
        ...
        if (!string.IsNullOrEmpty(NotifyBody.fxddh))
        {
            #region 取得withdrawSerial

            withdrawSerial = NotifyBody.fxddh;
            #endregion
        ...
        try
        {
            ...
            if (DT != null && DT.Rows.Count > 0)
            {
                ...
                if (providerRequestData.IsQuerySuccess)
                {
                    ...
                    {
                        if (withdrawalModel.Amount != decimal.Parse(NotifyBody.fxfee))
                        {
                            PayDB.InsertPaymentTransferLog("金額不符", 3, "", providerSetting.ProviderCode);
                            response = Request.CreateResponse(HttpStatusCode.OK);
                            HttpContext.Current.Response.Write("amount error");
            ...
            string signStr = "fxid=" + providerSetting.MerchantCode +
                            "&fxddh=" + NotifyBody.fxddh +
                            "&fxorder=" + NotifyBody.fxorder +
                            "&fxstatus=" + NotifyBody.fxstatus +
                            "&fxfee=" + NotifyBody.fxfee +
                            "&fxdffee=" + NotifyBody.fxdffee +
                            "&fxbankaccount=" + NotifyBody.fxbankaccount +
                            "&fxname=" + NotifyBody.fxname +
                            "&fxbankname=" + NotifyBody.fxbankname +
                            "&key=" + secret;

            string sign = CodingControl.GetMD5(signStr, false);

            if (sign != NotifyBody.fxsign)
            {
                PayDB.InsertBotSendLog(providerSetting.ProviderCode, providerSetting.ProviderCode + "sign fail");
                response = Request.CreateResponse(HttpStatusCode.OK);
            ...
            {   //代付
                ...
                if (withdrawSuccess)
                {
                    ...
                    if (withdrawalModel.UpStatus != 2)
                    {
                        ...
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), NotifyBody.fxorder, decimal.Parse(NotifyBody.fxfee), withdrawSerial);
                ...
                else
                {
                    if (withdrawalModel.UpStatus != 2)
                    {
                        ...
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), NotifyBody.fxorder, decimal.Parse(NotifyBody.fxfee), 3, withdrawSerial);
                        returnStatus = GatewayCommon.WithdrawResultStatus.Failure;
                    }
                    else
                    {
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("AIPayNotify")]
    public HttpResponseMessage AIPayNotify([FromBody] _AIPayNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("AIPay");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        try
        {
            ...
            signDic.Add("mchKey", providerSetting.MerchantCode);
            signDic.Add("mchOrderNo", NotifyBody.mchOrderNo);
            signDic.Add("serialOrderNo", NotifyBody.serialOrderNo);
            signDic.Add("nonce", NotifyBody.nonce);

            signDic.Add("payStatus", NotifyBody.payStatus);
            signDic.Add("amount", NotifyBody.amount);
            signDic.Add("realAmount", NotifyBody.realAmount);
            signDic.Add("payTime", NotifyBody.payTime);


            signDic.Add("mchUserId", NotifyBody.mchUserId);
            signDic.Add("attach", NotifyBody.attach);
            signDic.Add("errorMsg", NotifyBody.errorMsg);
            ...
            #endregion
            #region 轉換status代碼
            if (NotifyBody.payStatus.ToUpper() == "SUCCESS")
            {//成功
                #region 反查訂單
                System.Data.DataTable DT = null;
                DT = PayDB.GetPaymentByPaymentID(paymentSerial);
            ...
            #endregion
            if (paymentModel.OrderAmount != decimal.Parse(NotifyBody.realAmount) /100)
            {
                PayDB.InsertPaymentTransferLog("金額不符", 3, "", providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.OK);
                HttpContext.Current.Response.Write("amount error");
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.realAmount)/100, NotifyBody.serialOrderNo))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("ryutaNotify")]
    public HttpResponseMessage ryutaNotify([FromBody] _ryutaNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("ryuta");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        if (NotifyBody != null)
        {
            ...
            paymentSerial = NotifyBody.data.transaction_id;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 3, paymentSerial, providerSetting.ProviderCode);
        }
        else
        ...
        try
        {
            ...
            string token = providerSetting.ProviderPublicKey;
            string secret = providerSetting.MerchantKey;
            string data = token + "amount=" + NotifyBody.data.amount+ "&payment_method=" + NotifyBody.data .payment_method+ "&request_amount=" + NotifyBody.data.request_amount+ "&status=" + NotifyBody.data.status+ "&transaction_id=" + NotifyBody.data.transaction_id+ "&transaction_reference=" + NotifyBody.data.transaction_reference;
            sign = CodingControl.GetHmacSHA256(secret, data);
            sign=sign.Replace('+', '-').Replace('/', '_');

            if (sign != NotifyBody.data.hash)
            {
                PayDB.InsertPaymentTransferLog("供应商完成订单通知,签名有误", 3, paymentSerial, providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.OK);
                HttpContext.Current.Response.Write("fail");
            ...
            if (NotifyBody.data.status.ToLower() == "completed")
            {//成功
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.data.amount), NotifyBody.data.transaction_reference))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("ryutaWithdrawNotify")]
    public HttpResponseMessage ryutaWithdrawNotify([FromBody] _ryutaWithdrawNotifyBody NotifyBody)
    {
        ...
        if (!string.IsNullOrEmpty(NotifyBody.data.transaction_id))
        {
            #region 取得withdrawSerial

            withdrawSerial = NotifyBody.data.transaction_id;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成代付订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 4, withdrawSerial, providerSetting.ProviderCode);
        }
        else
        ...
        try
        {
            ...
            string token = providerSetting.ProviderPublicKey;
            string secret = providerSetting.MerchantKey;
            string data = token + "account_number="+ NotifyBody.data.account_number + "&amount=" + NotifyBody.data.amount + "&bank_code=" + NotifyBody.data.bank_code  + "&status=" + NotifyBody.data.status + "&transaction_id=" + NotifyBody.data.transaction_id + "&transaction_reference=" + NotifyBody.data.transaction_reference;
            sign = CodingControl.GetHmacSHA256(secret, data);
            sign = sign.Replace('+', '-').Replace('/', '_');
            ...
            {   //代付
                ...
                if (withdrawSuccess)
                {
                    ...
                    {
                        //不修改Withdraw之狀態，預存中調整
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.data.amount), withdrawSerial);
                        var intReviewWithdrawal = PayDB.ReviewWithdrawal(withdrawSerial);
                        switch (intReviewWithdrawal)
                        {
                            case 0:
                ...
                else
                {
                    ...
                    {
                        PayDB.InsertPaymentTransferLog("订单完成,供应商通知此单失败,尚未通知商户", 4, withdrawSerial, providerSetting.ProviderCode);
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.data.amount), 3, withdrawSerial);
                        returnStatus = GatewayCommon.WithdrawResultStatus.Failure;
                    }
                    else
                    {
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("lanhaiPayNotify")]
    public HttpResponseMessage lanhaiPayNotify([FromBody] _lanhaiPayNotifyAsyncNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("lanhai");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        try
        {
            ...
            #endregion
            #region 轉換status代碼
            if (NotifyBody.status.ToUpper() == "2"|| NotifyBody.status.ToUpper() == "3")
            {//成功
                #region 反查訂單
                System.Data.DataTable DT = null;
                DT = PayDB.GetPaymentByPaymentID(paymentSerial);
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.amount)/100, NotifyBody.payOrderId))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("JBPWithdrawNotify")]
    public HttpResponseMessage JBPWithdrawNotify([FromUri] string type, [FromBody] _JBPWithdrawAsyncNotifyBody NotifyBody)
    {

        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("JBP");
        bool withdrawSuccess = false;
        string withdrawSerial = "";
        Dictionary<string, string> sendDic = new Dictionary<string, string>();
        GatewayCommon.Withdrawal withdrawalModel;
        ...
        if (!string.IsNullOrEmpty(NotifyBody.company_order_num))
        {
            #region 取得withdrawSerial

            withdrawSerial = NotifyBody.company_order_num;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成代付订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 4, withdrawSerial, providerSetting.ProviderCode);
        }
        else
        ...
        try
        {
           ...
           string sign = CodingControl.CalculateMD5(CodingControl.CalculateMD5(providerSetting.MerchantKey) + NotifyBody.mownecum_order_num + NotifyBody.company_order_num+ NotifyBody.status+ NotifyBody.amount+ NotifyBody.exact_transaction_charge);

            if (sign != NotifyBody.key)
            {
            ...
            if (DT != null && DT.Rows.Count > 0)
            {
                if (NotifyBody.status == "1" || NotifyBody.status == "0")
                {
                    withdrawalModel = GatewayCommon.ToList<GatewayCommon.Withdrawal>(DT).FirstOrDefault();
            ...
            else
            {
                ...
                PayDB.InsertBotSendLog(providerSetting.ProviderCode, providerSetting.ProviderCode + "反查订单失败,资料库内无此笔订单");
                response = Request.CreateResponse(HttpStatusCode.BadRequest);
                HttpContext.Current.Response.Write("order not exist");
                //HttpContext.Current.Response.Flush();
                //HttpContext.Current.Response.End();
                return response;
            }
            ...
            {   //代付
                ...
                if (withdrawSuccess)
                {
                    ...
                    {
                        //不修改Withdraw之狀態，預存中調整
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), NotifyBody.mownecum_order_num, decimal.Parse(NotifyBody.amount), withdrawSerial);
                        var intReviewWithdrawal = PayDB.ReviewWithdrawal(withdrawSerial);
                        switch (intReviewWithdrawal)
                        {
                            case 0:
                ...
                else
                {
                    ...
                    {
                        PayDB.InsertPaymentTransferLog("订单完成,供应商通知此单失败,尚未通知商户", 4, withdrawSerial, providerSetting.ProviderCode);
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), NotifyBody.mownecum_order_num, decimal.Parse(NotifyBody.amount), 3, withdrawSerial);
                        returnStatus = GatewayCommon.WithdrawResultStatus.Failure;
                    }
                    else
                    {
            ...
            HttpContext.Current.Response.Write("{\"status\":1}");
            //HttpContext.Current.Response.Flush();
            //HttpContext.Current.Response.End();
        }
        catch (Exception ex)
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("axecedddPayNotify")]
    public HttpResponseMessage axecedddPayNotify([FromBody] _axecedddPayAsyncNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("axecedddPay");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        JObject data = JsonConvert.DeserializeObject<JObject>(NotifyBody.data);
  
        if (data["order"]["merchant_order_id"] != null)
        {
            #region 取得PaymentID
            paymentSerial = data["order"]["merchant_order_id"].ToString();
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 3, paymentSerial, providerSetting.ProviderCode);
        }
        else
        {
            PayDB.InsertPaymentTransferLog(JsonConvert.SerializeObject(NotifyBody), 3, "", providerSetting.ProviderCode);
            PayDB.InsertPaymentTransferLog("供应商完成订单通知有误", 3, "", providerSetting.ProviderCode);
            response = Request.CreateResponse(HttpStatusCode.OK);
            HttpContext.Current.Response.Write("fail");
        ...
        try
        {
            ...
            string xmlPublicKey = providerSetting.OtherDatas[0];
            PayDB.InsertPaymentTransferLog(xmlPublicKey, 3, "", providerSetting.ProviderCode);
            // 创建RSA实例并加载公钥
            RSACryptoServiceProvider rsa = new RSACryptoServiceProvider();
            rsa.FromXmlString(xmlPublicKey);

            byte[] dataBytes = Encoding.UTF8.GetBytes(NotifyBody.data);
            SHA256Managed sha256 = new SHA256Managed();
            byte[] hashedData = sha256.ComputeHash(dataBytes);

            byte[] signatureBytes = Convert.FromBase64String(NotifyBody.signature);
  

            // 使用SHA256哈希算法进行签名验证
            bool verified = rsa.VerifyHash(hashedData, CryptoConfig.MapNameToOID("SHA256"), signatureBytes);
            ...
            if (data["order"]["status"].ToString().ToUpper() == "COMPLETED"&& data["notify_type"].ToString().ToUpper() == "TRADE_COMPLETED")
            {//成功
                #region 反查訂單
                System.Data.DataTable DT = null;
                DT = PayDB.GetPaymentByPaymentID(paymentSerial);
            ...
            #endregion
            if (paymentModel.OrderAmount != decimal.Parse(data["order"]["total_amount"].ToString()))
            {
                PayDB.InsertPaymentTransferLog("金額不符", 3, "", providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.OK);
                HttpContext.Current.Response.Write("amount error");
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(data["order"]["total_amount"].ToString()), data["order"]["id"].ToString()))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("SGPayNotify")]
    public HttpResponseMessage SGPayNotify([FromBody] _SGPayNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("SGPay");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        try
        {
            ...
            {
                // 获取 Signature 标头的值
                 signatureValue = request.Headers.GetValues("Signature").FirstOrDefault();
            }
            else
            {
                PayDB.InsertPaymentTransferLog("供应商完成订单通知,Signature 為空", 3, paymentSerial, providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.BadRequest);
                HttpContext.Current.Response.Write("fail");
            }
            ...
            signDic.Add("txid", NotifyBody.txid);
            signDic.Add("type", NotifyBody.type);
            signDic.Add("customer_uid", NotifyBody.customer_uid);
            signDic.Add("request_amount", NotifyBody.request_amount);
            signDic.Add("currency", NotifyBody.currency);
            signDic.Add("actual_amount", NotifyBody.actual_amount);
            signDic.Add("status", NotifyBody.status);
            signDic.Add("method", NotifyBody.method);

            signDic = CodingControl.AsciiDictionary(signDic);


            string hashCode = CodingControl.CalculateMD5(providerSetting.MerchantKey+JsonConvert.SerializeObject(signDic));


            if (hashCode != signatureValue)
            {
                PayDB.InsertPaymentTransferLog("供应商完成订单通知,签名有误", 3, paymentSerial, providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.BadRequest);
                HttpContext.Current.Response.Write("fail");
            ...
            #endregion
            #region 轉換status代碼
            if (NotifyBody.status.ToUpper() == "COMPLETED")
            {//成功
                #region 反查訂單
                System.Data.DataTable DT = null;
                DT = PayDB.GetPaymentByPaymentID(paymentSerial);
            ...
            else if (NotifyBody.status.ToUpper() == "EXPIRED")
            {
                response = Request.CreateResponse(HttpStatusCode.OK);
            ...
            #endregion
            if (paymentModel.OrderAmount != decimal.Parse(NotifyBody.actual_amount))
            {
                PayDB.InsertPaymentTransferLog("金額不符", 3, "", providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.BadRequest);
                HttpContext.Current.Response.Write("amount error");
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.actual_amount), ""))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("ucpayNotify")]
    public HttpResponseMessage ucpayNotify([FromBody] _ucpayNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("ucpay");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        if (NotifyBody != null)
        {
            ...
            paymentSerial = NotifyBody.order_number;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 3, paymentSerial, providerSetting.ProviderCode);
        }
        else
        ...
        try
        {
            ...
            signDic.Add("merchant_number", providerSetting.MerchantCode);
            signDic.Add("order_number", NotifyBody.order_number);
            ...
            #endregion
            #region 轉換status代碼
            if (NotifyBody.status.ToUpper() == "2"|| NotifyBody.status.ToUpper() == "6")
            {//成功
                #region 反查訂單
                System.Data.DataTable DT = null;
                DT = PayDB.GetPaymentByPaymentID(paymentSerial);
            ...
            #endregion
            if (paymentModel.OrderAmount != decimal.Parse(NotifyBody.order_amount))
            {
                PayDB.InsertPaymentTransferLog("金額不符", 3, "", providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.BadRequest);
                HttpContext.Current.Response.Write("amount error");
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.order_amount), ""))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("wanliuPayNotify")]
    public HttpResponseMessage wanliuPayNotify([FromBody] _wanliuPayNotifyAsyncNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("wanliuPay");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        if (NotifyBody != null)
        {
            ...
            paymentSerial = NotifyBody.orderId;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 3, paymentSerial, providerSetting.ProviderCode);
        }
        else
        ...
        try
        {
            ...
            signDic.Add("merchantId", providerSetting.MerchantCode);
            signDic.Add("orderId", NotifyBody.orderId);
            signDic.Add("amount", NotifyBody.amount);
            signDic.Add("status", NotifyBody.status);
            ...
            #endregion
            #region 轉換status代碼
            if (NotifyBody.status.ToUpper() == "OK")
            {//成功
                #region 反查訂單
                System.Data.DataTable DT = null;
                DT = PayDB.GetPaymentByPaymentID(paymentSerial);
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.amount), NotifyBody.orderId))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("EVOpayNotify")]
    public HttpResponseMessage EVOpayNotify([FromBody] _EVOpayNotifyAsyncNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("EVOpay");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        if (NotifyBody != null)
        {
            ...
            paymentSerial = NotifyBody.out_trade_no;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 3, paymentSerial, providerSetting.ProviderCode);
        }
        else
        ...
        try
        {
            ...
            signDic.Add("amount", NotifyBody.amount);
            signDic.Add("body", NotifyBody.body);
            signDic.Add("channel", NotifyBody.channel);
            signDic.Add("trade_no", NotifyBody.trade_no);
            signDic.Add("out_trade_no", NotifyBody.out_trade_no);
            signDic.Add("order_status", NotifyBody.order_status);
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.amount), NotifyBody.trade_no))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("EShopPayNotify")]
    public HttpResponseMessage EShopPayNotify([FromBody] _EShopPayAsyncNotifyBody NotifyBody)
    {
        GatewayCommon.ProviderSetting providerSetting = GatewayCommon.GetProverderSettingData("EShopPay");
        GatewayCommon.Payment paymentModel;
        int processStatus;
        HttpResponseMessage response;
        bool companyRequestResult = false;
        ...
        if (NotifyBody != null)
        {
            ...
            paymentSerial = NotifyBody.ClientOrderNumber;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 3, paymentSerial, providerSetting.ProviderCode);
        }
        else
        ...
        try
        {
            ...
            signDic.Add("StoreCode", providerSetting.MerchantCode);
            signDic.Add("PaymentSerial", NotifyBody.PaymentSerial);
            signDic.Add("FinishDate", NotifyBody.FinishDate);
            signDic.Add("Amount",Decimal.Parse(NotifyBody.Amount).ToString("#.##"));
            signDic.Add("PaymentStatus", NotifyBody.PaymentStatus);
            signDic.Add("Currency", NotifyBody.Currency);
            signDic.Add("ClientOrderNumber", NotifyBody.ClientOrderNumber);

            foreach (var property in signDic.OrderBy(x => x.Key))
            {
                stringBuilder.Append(property.Key + "=" + property.Value + "&");
            }

            stringBuilder.Append("Key" + "=" + providerSetting.MerchantKey);
            ...
            if (paymentModel.OrderAmount != decimal.Parse(NotifyBody.ReceiveAmount))
            {
                PayDB.InsertPaymentTransferLog("金額不符", 3, "", providerSetting.ProviderCode);
                response = Request.CreateResponse(HttpStatusCode.OK);
                HttpContext.Current.Response.Write("amount error");
            ...
            //寫入預存
            switch (PayDB.SetPaymentProcessStatus(paymentSerial, processStatus, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.ReceiveAmount), NotifyBody.PaymentSerial))
            {
                case 0:
                    //撈取該單，準備回傳資料
                    response = Request.CreateResponse(HttpStatusCode.OK);
    ...
    [HttpGet]
    [HttpPost]
    [ActionName("EShopWithdrawNotify")]
    public HttpResponseMessage EShopWithdrawNotify([FromBody] _EShopPayAsyncNotifyBody NotifyBody)
    {
        ...
        if (!string.IsNullOrEmpty(NotifyBody.ClientOrderNumber))
        {
            #region 取得withdrawSerial

            withdrawSerial = NotifyBody.ClientOrderNumber;
            #endregion
            PayDB.InsertPaymentTransferLog("供应商完成代付订单通知,回调IP:" + ProxyIP + ",资料:" + JsonConvert.SerializeObject(NotifyBody), 4, withdrawSerial, providerSetting.ProviderCode);
        }
        else
        ...
        string sign = CodingControl.GetMD5(stringBuilder.ToString(), false).ToLower();
        ...
        try
        {
            ...
            if (DT != null && DT.Rows.Count > 0)
            {
                ...
                if (NotifyBody.PaymentStatus == "1")
                {
                    if (withdrawalModel.Amount != decimal.Parse(NotifyBody.ReceiveAmount))
                    {
                        PayDB.InsertPaymentTransferLog("金額不符", 3, "", providerSetting.ProviderCode);
                        response = Request.CreateResponse(HttpStatusCode.OK);
                        HttpContext.Current.Response.Write("amount error");
                ...
                else if (NotifyBody.PaymentStatus == "2")
                {
                    withdrawSuccess = false;
                }
                else {
            ...
            {
                PayDB.InsertPaymentTransferLog("反查订单失败,资料库内无此笔订单:"+ withdrawSerial, 4, "", providerSetting.ProviderCode);
            ...
            {   //代付
                ...
                if (withdrawSuccess)
                {
                    ...
                    {
                        //不修改Withdraw之狀態，預存中調整
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.ReceiveAmount), withdrawSerial);
                        var intReviewWithdrawal = PayDB.ReviewWithdrawal(withdrawSerial);
                        switch (intReviewWithdrawal)
                        {
                            case 0:
                ...
                else
                {
                    ...
                    {
                        PayDB.InsertPaymentTransferLog("订单完成,供应商通知此单失败,尚未通知商户", 4, withdrawSerial, providerSetting.ProviderCode);
                        PayDB.UpdateWithdrawSerialByUpData(2, JsonConvert.SerializeObject(NotifyBody), "", decimal.Parse(NotifyBody.ReceiveAmount), 3, withdrawSerial);
                        returnStatus = GatewayCommon.WithdrawResultStatus.Failure;
                    }
                    else
                    {
    ...
    {
        PayDB.InsertPaymentTransferLog(JsonConvert.SerializeObject(result), 4, "TEST", "TESTCODE");
        var response = Request.CreateResponse(HttpStatusCode.OK);
        HttpContext.Current.Response.Write("SUCCESS");
        HttpContext.Current.Response.Flush();
        HttpContext.Current.Response.End();
    ...
    public class _JBPWithdrawAsyncNotifyBody
    {
        public string status { get; set; }
        public string amount { get; set; }
        ...
        public string exact_transaction_charge { get; set; }
        public string key { get; set; }
    }

    public class _axecedddPayAsyncNotifyBody
    {
        public string data { get; set; }
        public string signature { get; set; }
    }

    public class _axecedddPayAsyncNotifyData
    {
    ...
    public class _EVOpayNotifyAsyncNotifyBody
    {
        public string amount { get; set; }
    ...
    public class _EShopPayAsyncNotifyBody
    {
    ...
    public class _JiuYuanPayAsyncNotifyBody
    {
        public string fxid { get; set; }
        public string fxddh { get; set; }
        public string fxorder { get; set; }
        public string fxdesc { get; set; }
        public string fxfee { get; set; }
        public string fxattch { get; set; }
        public string fxusername { get; set; }

        public string fxcardnumber { get; set; }
        public string fxstatus { get; set; }
        public string fxtime { get; set; }
        public string fxsign { get; set; }
    }

    public class _JiuYuanPayWAsyncNotifyBody
    {
        public string fxid { get; set; }
        public string fxddh { get; set; }
        public string fxorder { get; set; }
        public string fxfee { get; set; }
        public string fxdffee { get; set; }
        public string fxbankaccount { get; set; }
        public string fxname { get; set; }

        public string fxbankname { get; set; }
        public string fxzhihang { get; set; }
        public string fxsheng { get; set; }
        public string fxshi { get; set; }
        public string fxstatus { get; set; }
        public string fxdesc { get; set; }
        public string fxsign { get; set; }
    }
    ...
    public class _ryutaNotifyBody
    {
        public _ryutaNotifyData data { get; set; }
    }

    public class _ryutaNotifyData
    {
        public string transaction_id { get; set; }
        ...
        public string message { get; set; }
        public string status { get; set; }
        public JObject payment_info { get; set; }
        public string created_at { get; set; }
        public string completed_at { get; set; }
        public string createhashd_at { get; set; }
        public string hash { get; set; } 
    }

    public class _ryutaWithdrawNotifyBody
    {
        public _ryutaWithdrawNotifyData data { get; set; }
    }

    public class _ryutaWithdrawNotifyData
    {
        public string transaction_id { get; set; }
        public string transaction_reference { get; set; }
    ...
    public class _lanhaiPayNotifyAsyncNotifyBody
    {
        public string payOrderId { get; set; }
        public string mchId { get; set; }
        public string productId { get; set; }
    ...
    public class _wanliuPayNotifyAsyncNotifyBody
    {
    ...
    public class _AIPayNotifyBody
    {
        public string mchKey { get; set; }
        public string mchOrderNo { get; set; }
        public string serialOrderNo { get; set; }
